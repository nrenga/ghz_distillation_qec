...
for eps_iter = 1:length(epsilons)
    ...
    while((iter < blocks_min) || ((miscorrection(eps_iter) + syndrome_unmatched(eps_iter)) < no_of_logical_errors))
        ...
        [e_X_dec, s_X_dec, success_X_dec] = syndrome_MSA_seq_vars_5(H_Z, H_Z_vn_neighbors, H_Z_cn_neighbors, s_X, llr_X, max_iter);
        [e_Z_dec, s_Z_dec, success_Z_dec] = syndrome_MSA_seq_vars_5(H_X, H_X_vn_neighbors, H_X_cn_neighbors, s_Z, llr_Z, max_iter);
