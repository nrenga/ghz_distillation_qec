function [Xbar, Zbar] = find_logical_paulis_by_ghz_msmt(Hin)
...
% Simulate the creation of n GHZ states, omit the IZZ type stabilizers
...
Sout = S_GHZ;
for i = 1:(n-k)
    [Sout, replaced_rows(i)] = stabilizer_formalism_msmt(Sout, [H(i,1:n), zeros(1,2*n),  H(i,(n+1):2*n), zeros(1,2*n),  H(i,end)]);
end

% Determine logical Z and X operators
Zbar = [ zeros(k,2*n), ones(k,1) ];
inds = setdiff(1:n, replaced_rows);
logZ_ind = 1;
Hfull = H;
for i = 1:length(inds)
    Z_A = Sout(inds(i), (3*n+1):4*n);
    if (gfrank([Hfull(:,1:2*n); zeros(1,n), Z_A], 2) > gfrank(Hfull(:,1:2*n), 2))
        Zbar(logZ_ind, :) = [zeros(1,n), Z_A, 1];
        Hfull(n-k+logZ_ind, :) = [zeros(1,n), Z_A,  1];
        logZ_ind = logZ_ind + 1;
    end
end

Xbar = [ zeros(k,2*n), ones(k,1) ];
inds = setdiff((n+1):2*n, replaced_rows);
logX_ind = 1;
for i = 1:length(inds)
    X_A = [Sout(inds(i), 1:n) ,  Sout(inds(i), (3*n+1):4*n)];
    if (gfrank([Hfull(:,1:2*n); X_A], 2) > gfrank(Hfull(:,1:2*n), 2))
        Xbar(logX_ind, :) = [X_A, Sout(inds(i), end)];
        Hfull(n+logX_ind, :) = [X_A, Sout(inds(i), end)];
        logX_ind = logX_ind + 1;
    end
end

% Match the rows of Zbar accordingly with Xbar
symp_inn = mod(Zbar(:,1:2*n) * fftshift(Xbar(:,1:2*n),2)', 2);
if (norm(symp_inn - eye(k),'fro') > 1e-10)
    symp_inn_inv = gf2matinv(symp_inn);
    Zbar(:,1:2*n) = mod(symp_inn_inv * Zbar(:,1:2*n), 2);
end
